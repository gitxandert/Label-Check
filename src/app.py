...
from typing import List, Dict, Any, Optional, Tuple, Counter as CounterType, Union

from flask import (
    Flask, render_template, request, redirect, url_for, send_from_directory,
    flash, session, jsonify, Response, get_flashed_messages
)
...
class Config:
    ...
    ADMIN_DEFAULT_PASSWORD = os.environ.get('ADMIN_DEFAULT_PASSWORD', 'change_this_password')
...
class QueueItem(db.Model):
    ...
    status = db.Column(db.String(20), nullable=False, default='pending', index=True)  # pending, leased, completed
    ...
    leased_by = db.relationship('User', foreign_keys=[leased_by_id], backref=db.backref('leases', lazy=True))
    completed_by = db.relationship('User', foreign_keys=[completed_by_id], backref=db.backref('completed_items', lazy=True))
...
def _release_expired_leases():
    ...
    expired_items = QueueItem.query.filter(
        ...
        QueueItem.leased_at < expired_time
    ).all()
    
    if expired_items:
        for item in expired_items:
            app.logger.info(f"Lease for item {item.original_index} expired (leased by {item.leased_by_id}). Releasing.")
            item.status = 'pending'
            item.leased_by_id = None
            item.leased_at = None
        db.session.commit()
        flash(f"{len(expired_items)} item(s) had expired leases and were returned to the queue.", "warning")
...
def _recalculate_accession_counts() -> None:
    ...
    id_counts: CounterType[str] = Counter(
        row.get('AccessionID', '').strip() for row in data if row.get('AccessionID', '').strip()
    )
    for row in data:
...
def get_display_info_for_original_index(original_index: int) -> Optional[Dict[str, int]]:
    ...
    try:
        ...
        app.logger.warning(f"Original index {original_index} not found in current display list.")
        return None
    ...
    try:
        with open(file_path, 'r', newline='', encoding='utf-8') as csvfile:
            ...
            if not _headers or not all(h in _headers for h in required_headers):
                raise DataLoadError(f"CSV file is missing one of the required headers: {required_headers}")
    ...
    try:
        with open(temp_path, 'w', newline='', encoding='utf-8') as csvfile:
            writer = csv.DictWriter(
                csvfile, fieldnames=write_headers, delimiter=';',
        ...
        if os.path.exists(temp_path):
            try:
                ...
                app.logger.error(f"Failed to remove temporary save file {temp_path}: {remove_err}")
...
def _create_backup() -> None:
    ...
    try:
        ...
        backup_path = os.path.join(Config.BACKUP_DIR, f"{base_filename}_{timestamp}.bak")
...
def before_request_handler() -> None:
    ...
    if not data:
        ...
        try:
            ...
            app.logger.error(f"Main CSV file '{Config.CSV_FILE_PATH}' disappeared. Cannot check mod time.")
            flash(f"CRITICAL: Main CSV file '{Config.CSV_FILE_PATH}' not found.", "critical")
            return
        except Exception as e:
...
def users_management() -> Union[Response, str]:
    ...
    if not current_user.is_admin:
        app.logger.warning(f"User '{current_user.id}' attempted to access admin page without permission.")
...
def add_user() -> Response:
    ...
    if not current_user.is_admin:
        app.logger.warning(f"User '{current_user.id}' attempted to add a user without permission.")
...
def index() -> str:
    ...
    if requested_index_str:
        try:
            ...
            active_lease = QueueItem.query.filter_by(leased_by_id=current_user.id, status='leased').first()
            if active_lease:
            ...
            item_to_display = QueueItem.query.filter_by(original_index=requested_index).first()
            if not item_to_display:
                flash("The requested item is not in the queue.", "error")
                return redirect(url_for('index'))

            if item_to_display.status == 'leased' and item_to_display.leased_by_id != current_user.id:
                flash(f"Item with Accession ID: {data[item_to_display.original_index]['AccessionID']} is leased by another user. Viewing in read-only mode.", "warning")
            else:
                ...
                db.session.commit()
                flash(f"Viewing item with Accession ID: {data[item_to_display.original_index]['AccessionID']}.", "info")
        ...
        if active_lease:
            ...
            flash(f"You are currently working on item with Accession ID: {data[item_to_display.original_index]['AccessionID']}. Don't forget to save or release it.", "info")
        else:
            # Get the next available item from the queue
            item_to_display = QueueItem.query.filter_by(status='pending').order_by(QueueItem.original_index).first()
            ...
            if item_to_display:
                ...
                item_to_display.leased_at = datetime.datetime.utcnow()
                db.session.commit()
                app.logger.info(f"User '{current_user.id}' leased item {item_to_display.original_index}.")
                flash(f"Assigned new item with Accession ID: {data[item_to_display.original_index]['AccessionID']} from the queue.", "success")
            else:
                ...
                return render_template('index.html', no_items_left=True, completed_count=completed_count, total_count=total_count, messages=flash_messages())
    ...
    recently_completed_items = QueueItem.query.filter_by(completed_by_id=current_user.id).order_by(QueueItem.completed_at.desc()).limit(5).all()
...
def update() -> Response:
    ...
    try:
        ...
        if not queue_item or queue_item.leased_by_id != current_user.id:
            flash("You do not hold the lease for this item. It may have expired or been taken by another user.", "error")
            return redirect(url_for('index'))
        ...
        if data_changed:
            app.logger.info(f"User '{current_user.id}' updated row index {original_index}.")
            try:
                current_user.correction_count += 1
                db.session.commit()
            except Exception as e:
                db.session.rollback()
                app.logger.error(f"Failed to update correction count for user '{current_user.id}': {e}")
        ...
        if data_changed:
            try:
                _create_backup()
                save_csv_data()
                flash(f"Changes automatically saved to {os.path.basename(Config.CSV_FILE_PATH)}.", "success")
            except BackupError as e:
                app.logger.error(f"SAVE FAILED due to backup error: {e}")
                flash(f"Save cancelled: Backup failed. Changes are not saved to disk. Error: {e}", "error")
            except DataSaveError as e:
                app.logger.critical(f"DATA SAVE FAILED: {e}")
                flash(f"CRITICAL: Data saving FAILED. See logs for details. Error: {e}", "critical")
...
def release_lease() -> Response:
    ...
    active_lease = QueueItem.query.filter_by(leased_by_id=current_user.id, status='leased').first()
    if active_lease:
        app.logger.info(f"User '{current_user.id}' manually released lease for item {active_lease.original_index}.")
        active_lease.status = 'pending'
        active_lease.leased_by_id = None
        active_lease.leased_at = None
        db.session.commit()
        flash(f"Lease for item {active_lease.original_index + 1} has been released.", "info")
    else:
...
def history() -> str:
    ...
    completed_items = QueueItem.query.filter_by(completed_by_id=current_user.id).order_by(QueueItem.completed_at.desc()).all()
    
    # Add AccessionID to completed_items
    for item in completed_items:
        item.accession_id = data[item.original_index]['AccessionID']

    return render_template('history.html', completed_items=completed_items, messages=flash_messages())
...
def jump() -> Response:
    ...
    try:
        ...
        if 0 <= jump_target_0based < len(display_indices):
            ...
            flash(f"Invalid jump target: {jump_target_1based}. Please enter a number between 1 and {len(display_indices)}.", "error")
            return redirect(url_for('index', index=current_index))
...
def search() -> Response:
    ...
    for i, row in enumerate(data):
        if row.get('AccessionID', '').lower() == search_term or row.get('_identifier', '').lower() == search_term:
...
if __name__ == '__main__':
    with app.app_context():
        ...
        if not os.path.exists(db_path):
            app.logger.warning(f"Database not found at {db_path}. Please run 'flask init-db' to create it.")
            print("\nWARNING: Database not found.")
            print(f"Please run the following command in your terminal to initialize it:")
            print("flask init-db\n")
            exit(1)
    ...
    try:
        ...
        app.logger.critical(f"Initial data load failed: {e}. The application cannot start.")
        print(f"\nCRITICAL: Initial data load failed: {e}")
        print("The application cannot start without the data file. Please check the path and file integrity.")
        exit(1)
